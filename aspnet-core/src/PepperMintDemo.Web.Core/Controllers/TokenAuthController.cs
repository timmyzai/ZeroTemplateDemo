...
using PepperMintDemo.Authentication.TwoFactor.Google;
using PepperMintDemo.Authorization;
using PepperMintDemo.Authorization.Accounts.Dto;
...
namespace PepperMintDemo.Web.Controllers
{
    ...
    public class TokenAuthController : PepperMintDemoControllerBase
    {
        ...
        private async Task<string> EncryptQueryParameters(long userId, string passwordResetCode)
        {
            ...
            var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours).ToString(PepperMintDemoConsts.DateTimeOffsetFormat));
