...
using PepperMintDemo.UiCustomization.Dto;

namespace PepperMintDemo.Web.UiCustomization.Metronic
{
        ...
        private void ValidateSettings(ThemeSettingsDto settings)
        {
            if (settings.Menu.AllowAsideMinimizing && !settings.Menu.HoverableAside)
            {
                throw new UserFriendlyException(
                    _localizationManager.GetSource(PepperMintDemoConsts.LocalizationSourceName)
